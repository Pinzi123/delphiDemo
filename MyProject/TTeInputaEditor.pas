...
if CheckValue and (EditData.State in [dsInsert,dsEdit]) then
   begin
     if(PurStr <> '')  then
     begin
       TempString:='SELECT Item_No,Mat_No,Vendor,Price,Mat_Desc,Mat_Name,Req_Pur_Qty,Unit FROM Te_Purchasea WHERE Pur_No='''+PurStr+''''
     ...
     begin
       TempString:='SELECT Item_No,Mat_No,Mat_Desc,Mat_Name,Vendor,Req_Qty,Price,Unit FROM Te_Lista WHERE List_No='''+ListStr+''''
